...
// ----------------------------------------------------------------------------------------------------------
log_file::log_file(switch_mode mode_, const string& logpfx_, const string& procname_)
	: procname(procname_),
	  logpfx(logpfx_)
{
   if (procname.empty())
...
void log_file::setenv()
{
	 char *ptr = ::getenv("LOG_LEVEL");
	if (ptr == NULL)
		log_lvl = -1;
	else
		log_lvl = ::atoi(ptr);
...
void log_file::set_procname(const string& procname_)
{
	procname = procname_;
	pid = ::getpid();
}
...
void log_file::write_log(const char* filename, int linenum, const char* msg, int len)
{
	...
	if (fp == 0) {
		cerr << (_("ERROR: Open file {1} error, [{2}] : {3}\n") % log_name % date_time % msg ).str(SGLOCALE);
	} else {
		fprintf(fp,"[%s] [%s] [%ld] : ",date_time,procname.c_str(),static_cast<long>(pid));
...
void log_file::write_log(const char *msg)
{
	char date_time[32];
	time_t t = time(0);
	tm tm;
...
void log_file::write_log(const char* filename, int linenum, const string& msg)
{
	char date_time[32];
	time_t t = time(0);
	tm tm;
...
void log_file::write_log(int lvl, const char* filename, int linenum, const char* msg, int len)
{
	if (lvl >= log_lvl)
		write_log(filename, linenum, msg, len);
}

void log_file::write_log(int lvl, const char* filename, int linenum, const string& msg)
{
	if (lvl >= log_lvl)
		write_log(filename, linenum, msg);
}

void log_file::write_log(int lvl, const string& msg)
{
	if (lvl >= log_lvl)
		write_log(msg);
}

void log_file::write_log(int lvl, const char *msg)
{
	if (lvl >= log_lvl)
		write_log(msg);
}
...
{
	return client->wait(ipsocket::READ_POLL, sec, usec);
}
