...
void ftp::pwd(string& path)
{
    ...
    if (s == 0)
        throw bad_file(__FILE__, __LINE__, 173, bad_file::bad_sock, (_("ERROR: failed to execute command PWD")).str(SGLOCALE));
    s++;
    path = "";
    while (*s && (*s != '"'))
        path += *s++;
...
void ftp::size(const string& path, int& size, char mode)
{
    ...
    if (sscanf(net_ctrl.response, "%d %d", &resp, &size) != 2)
        throw bad_file(__FILE__, __LINE__, 173, bad_file::bad_sock, (_("ERROR: failed to execute command SIZE")).str(SGLOCALE));
}
...
void ftp::close()
{
    ...
    if (net_data.dir != FTPLIB_WRITE && net_data.dir != FTPLIB_READ)
        throw bad_file(__FILE__, __LINE__, 173, bad_file::bad_sock, (_("ERROR: Invalid mode")).str(SGLOCALE));
    shutdown(net_data.handle, 2);
    ::close(net_data.handle);
    net_data.handle = -1;
	if (net_data.ctrl && !read_resp('2')){
		status = LOGINED;
		throw bad_file(__FILE__, __LINE__, 173, bad_file::bad_sock, (_("ERROR: Invalid response")).str(SGLOCALE));
	}
    status = LOGINED;
}
...
int ftp::read_line(char* buf, int max, netbuf* ctl)
{
    ...
    {
        throw bad_param(__FILE__, __LINE__, 174,(_("ERROR: Invalid to read line")).str(SGLOCALE));
    }

    if (ctl->dir != FTPLIB_CONTROL && ctl->dir != FTPLIB_READ)
        throw bad_file(__FILE__, __LINE__, 173, bad_file::bad_sock, (_("ERROR: Invalid direction")).str(SGLOCALE));
...
void ftp::send_cmd(const string& cmd, char expresp)
{
    ...
    if (net_ctrl.dir != FTPLIB_CONTROL)
        throw bad_file(__FILE__, __LINE__, 173, bad_file::bad_sock, (_("ERROR: Invalid direction")).str(SGLOCALE));
    dout << "cmd = [" << cmd << "]" << std::endl;
    if (*cmd.rbegin() == '\n')
        throw bad_param(__FILE__, __LINE__, 411, (_("ERROR: cmd token with return character")).str(SGLOCALE));
    cmd_tmp = cmd + "\r\n";
    ...
    if (!read_resp(expresp))
        throw bad_file(__FILE__, __LINE__, 173, bad_file::bad_sock, (_("ERROR: read_resp: {1}") % cmd).str(SGLOCALE));
}
...
void ftp::open_port(char mode, int dir)
{
    ...
    if (net_ctrl.dir != FTPLIB_CONTROL)
        throw bad_file(__FILE__, __LINE__, 173, bad_file::bad_sock, (_("ERROR: open_port")).str(SGLOCALE));
    ...
    if (net_ctrl.cmode == FTPLIB_PASSIVE)
    {
        ...
        if (cp == 0)
            throw bad_file(__FILE__, __LINE__, 173, bad_file::bad_sock, (_("ERROR: failed to execute command PASV")).str(SGLOCALE));
        cp++;
    ...
    if (data == -1)
	    throw bad_file(__FILE__, __LINE__, 173, bad_file::bad_sock, (_("ERROR: Can't call socket()")).str(SGLOCALE));
...
void ftp::accept()
{
    ...
    else if (i == 0)
    {
        ::close(net_data.handle);
        net_data.handle = -1;
        throw bad_file(__FILE__, __LINE__, 173, bad_file::bad_sock, (_("ERROR: socket timeout")).str(SGLOCALE));
    }
    else
    {
        if (FD_ISSET(net_data.handle, &mask))
        {
            ...
            {
	        	net_data.handle = -1;
	        	throw bad_file(__FILE__, __LINE__, 173, bad_file::bad_sock, (_("ERROR: FD_ISSET")).str(SGLOCALE));
            }
        }
        ...
        while ((l = fread(dbuf, 1, FTPLIB_BUFSIZ, local)) > 0)
        {
            if (!write(dbuf, l))
            {
                if (!local_file.empty())
                    fclose(local);
                close();
                throw bad_file(__FILE__, __LINE__, 124, bad_file::bad_write, (_("ERROR: write")).str(SGLOCALE));
            }
        }
    }
    else
...
void ftp::access(const string& path, int typ, char mode)
{
    ...
    if (path.empty() && ((typ == FTPLIB_FILE_WRITE) || (typ == FTPLIB_FILE_READ))){
        status = LOGINED;
        throw bad_file(__FILE__, __LINE__, 173, bad_file::bad_sock, (_("ERROR: Missing path argument")).str(SGLOCALE));
    }
    fmt << "TYPE " << mode;
    try{
    	send_cmd(fmt.str(), '2');
    ...
    default:
    	status = LOGINED;
        throw bad_file(__FILE__, __LINE__, 173, bad_file::bad_sock, (_("ERROR: Invalid open type")).str(SGLOCALE));
    }
    if (!path.empty())
    {
        cmd += ' ';
