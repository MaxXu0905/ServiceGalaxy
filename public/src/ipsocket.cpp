/**
 * @file ipsocket.cpp
 * @brief The implement of the ipsocket class.
 *
 * @author Dejin Bao
...
void ipsocket::get_local_addr(char* localaddr)
{
    ...
    if ( localaddr == 0 || strlen(localaddr) < INET_ADDRSTRLEN + 1 )
        throw bad_file(__FILE__, __LINE__, 219, bad_file::bad_sock, (_("ERROR: localaddr is null or localaddr length must be more than 16")).str(SGLOCALE));
...
void ipsocket::get_remote_addr(char* remoteaddr)
{
    ...
    if ( remoteaddr == 0 || strlen(remoteaddr) < INET_ADDRSTRLEN )
        throw bad_file(__FILE__, __LINE__, 219, bad_file::bad_sock, (_("ERROR: remoteaddr is null or remoteaddr length must be more than 16")).str(SGLOCALE));
...
int ipsocket::read(void* buf, int maxsize, int readmode)
{
    ...
    while ( nleft > 0 )
    {
        ...
        {
            if (nread > maxsize)
                throw bad_msg(__FILE__, __LINE__, 219, (_("ERROR: recieve buffer is too small.")).str(SGLOCALE));
            nread += nret;
            nleft -= nret;
            if (readmode)
                break;
