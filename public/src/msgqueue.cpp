...
namespace sg
{
    ...
    int msgqueue::create_msgqueue()
    {
        ...
        if ( _nmsgid >= 0 ) {
            if ( ::msgctl(_nmsgid, IPC_RMID, 0) < 0 )
                throw bad_msg(__FILE__,__LINE__,220,(_("ERROR: Can't remove message queue {1}.") % _nmsgid).str(SGLOCALE));
            _nmsgid = -1;
        }

        while ( (_nmsgid= ::msgget(_nIpcKey, 0660 |IPC_CREAT)) < 0 ) {
            sleep(1);
            ntry ++;
            if ( ntry > 60 )
                throw bad_system(__FILE__,__LINE__,221,0,(_("ERROR: create message queue failed.")).str(SGLOCALE));
        }
        _nrunmode = 1;
    ...
    int msgqueue::get_msgqueue()
    {
        ...
        while ( (_nmsgid= ::msgget(_nIpcKey, 0600 )) < 0 ) {
            ...
            ntry ++;
            if ( ntry > 60 )
                throw bad_system(__FILE__,__LINE__,221,0,(_("ERROR: get message queue failed.")).str(SGLOCALE));
        }
        _nrunmode = 0;
        return _nmsgid > 0 ? 0 : -1;
    }
    ...
    int msgqueue::sendmsg(int nmsgtype, char cmd)
    {
        ...
        while ( ( nret = ::msgsnd(_nmsgid,(char*)&msg,sizeof(message) - sizeof(long),0)) < 0 ) {
            ntry ++;
            if ( ntry > 60 )
                throw bad_system(__FILE__,__LINE__,222,0,(_("ERROR: send message queue failed.")).str(SGLOCALE));
            sleep(1);
        }
        return nret;
    }
    ...
    int msgqueue::recvmsg(int nmsgtype,long& pid,char& cmd)
    {
        ...
        while ( ( nret = ::msgrcv(_nmsgid,(char*)&msg,sizeof(message),nmsgtype,0)) < 0 ) {
            ntry ++;
            if ( ntry > 60 )
                throw bad_system(__FILE__,__LINE__,223,0,(_("ERROR: recieve message queue failed.")).str(SGLOCALE));
            sleep(1);
        }
        pid = msg.mprocid;
        cmd = msg.mtext[0];
