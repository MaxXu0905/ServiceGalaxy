...
int main(int argc, char **argv)
{
	...
	try {
		...
		item.field_length = sizeof(long);
		column_desc.push_back(item);

		data->setSQL("select default_region_code, exp_date from bds_area_code where area_code = :area_code and eff_date = :eff_date;", column_desc);
		stmt->bind(data);
