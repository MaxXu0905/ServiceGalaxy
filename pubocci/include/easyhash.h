...
******************************************************************************
*  modify history
*  2007-8-24 增加hash散列函数定制接口,用户可以在构造函数中定制自己的散列函数,默认使用内置算法 刘羽
*  2007-8-28 增加迭代器支持,可以使用迭代器遍历HASH表中的数据,遍历方法与STL中容器一致.刘羽
*  2007-8-29 修改函数m_get_key，解决多线程查询冲突问题。刘羽
*  2007-12-14 增加lazy_iterator支持，增加成员函数 iterator begin(const UNIT& uni)，方便查询操作，尤其是不限返回个数的多重查询
*/

/*usage of easyhash

UNIT is a user defined struct type
example:
...
struct student
{
    ...
    student* next;
    void get_hash_key(char* hashkey) const
    {
        sprintf(hashkey, "%s", name);
    }

    bool hash_equal(const student& right)
    {
        return (strcmp(name, right.name) == 0);
    }
}

note the struct members
next, get_hash_key and hash_equal are necessary

结构体的成员避免使用string,vector,map等内部封装内存分配管理的模版类
因为hashtable内部的unit拷贝是使用memcpy赋值的
建议只使用标准数据类型
*/
...
typedef UNIT* pointer;
    ...
    {
        if (hash_size <= 0) {
            throw bad_param(__FILE__, __LINE__, 145, (_("ERROR: hash_size must bigger than 0.")).str(SGLOCALE));
        }
        if (max_data_count <= 0) {
            throw bad_param(__FILE__, __LINE__, 146, (_("ERROR: max_data_count must bigger than 0.")).str(SGLOCALE));
        }
    ...
    int search(const reference condition, pointer result, int max_result_count)
    {
        int ncount = 0;
        if (result == NULL) {
            throw bad_param(__FILE__, __LINE__, 108, (_("ERROR: resultbuf can not be null.")).str(SGLOCALE));
        }
        ...
        while (p != NULL) {
            if (p->hash_equal(condition)) {
                if (ncount >= max_result_count)
                    throw bad_msg(__FILE__, __LINE__, 109, (_("ERROR: max_result_count is too small.")).str(SGLOCALE));
    ...
    pointer insert(const reference unit)
    {
        ...
        if ((occup = alloc_record()) == NULL)
            throw bad_msg(__FILE__, __LINE__, 147, (_("ERROR: max_data_count is too small.")).str(SGLOCALE));
...
=            throw bad_msg(__FILE__, __LINE__, 103, (_("ERROR: Can not change hashmode for curr_data_count is not zero.")).str(SGLOCALE));
        }
        if (hashmode != NUMBER && hashmode != STRING) {
            throw bad_param(__FILE__, __LINE__, 104, (_("ERROR: unknown hashmode.")).str(SGLOCALE));
        }
    ...
    void set_calc_hash_func(calc_hash_func_t calchashfunc)
    {
        ...
        if (m_curr_data_count > 0) {
            throw bad_msg(__FILE__, __LINE__, (_("ERROR: Can not change calc_hash_func for curr_data_count is not zero.")).str(SGLOCALE));
        }
        m_calc_hash_func = calchashfunc;
        return;
...
private:
    ...
    long m_get_key(const reference unit)
    {
        ...
        if (hash_key[0] == 0) {
            throw bad_msg(__FILE__, __LINE__, 16, (_("ERROR: hash_key can not be null.")).str(SGLOCALE));
        }
        long ret;
        ...
        if (ret < 0) {
            throw bad_msg(__FILE__, __LINE__, 106, (_("ERROR: hash_key can not be a negative number.")).str(SGLOCALE));
        }
        return ret;
    };
    ...
    {
        if (m_hash_mode == NUMBER) {
        	return ::atol(key) % m_hash_size;
        } else {
