...
template <class Type> void pool<Type>::grow()
{
    ...
    {
        cerr << (_("Out of memory")).str(SGLOCALE) <<endl;
        exit(1);
    }
    n->next = blocks;
    blocks = n;
...
template <class Data> void  trie_tree<Data>::insert(Data data)
{
    ...
    while (key[i])
    {
        if (!isdigit(key[i]))
        {
            cerr  << (_("key must only contains char 0-9")).str(SGLOCALE) <<endl;
            return;
        }
        i++;
    }
...
/*
template <class Data> void trie_tree<Data>::remove(trie_node<Data> *node)
{
