        ...
        for( i = 0 ; i < MAX_MEM_BLKS ; i ++ )
        {
            if ( _pDataAddr[i] != 0 )
            {
                if ( ::shmdt(_pDataAddr[i]) < 0 )
                  throw bad_msg(__FILE__,__LINE__,282,(_("ERROR: shmdt failed.")).str(SGLOCALE));
                _pDataAddr[i] = 0;
            }
            if ( _pHashAddr[i] != 0 )
            {
...
int sharememclient::alloc_shm()
{
    ...
    while ( (shmid = ::shmget( _nIpcKey, 0, 0400 )) < 0 )
    {
        usleep(10);
        ntry ++;
        if ( ntry > 1000 )
          throw bad_msg(__FILE__,__LINE__,278,(_("ERROR: shmget failed, key = %1") % _nIpcKey).str(SGLOCALE));
    }
...
#else
    if ( _pobjshm <= 0 )
      throw bad_msg(__FILE__,__LINE__,282,(_("ERROR: attach super block share memory failed.")).str(SGLOCALE));
#endif
...
int sharememclient::alloc_object_mem()
{
    ...
    {
        while ( (shmid = ::shmget( _nIpcKey + 2 * MAX_MEM_BLKS + i, 0, 0400 )) < 0 )
        {
          usleep(10);
          ntry ++;
          if ( ntry > 1000 )
            throw bad_msg(__FILE__,__LINE__,283,(_("ERROR: shmget failed. key = {1}") % (_nIpcKey + 2 * MAX_MEM_BLKS + i)).str(SGLOCALE));
        }
        ...
        if ( _pDataAddr[i] == SHM_FAILED || _pDataAddr[i] == 0 )
          throw bad_msg(__FILE__,__LINE__,282,(_("ERROR: attach data share memory failed.")).str(SGLOCALE));
#else
        if ( _pDataAddr[i] < 0 )
          throw bad_msg(__FILE__,__LINE__,282,(_("ERROR: attach data share memory failed.")).str(SGLOCALE));
#endif
...
int sharememclient::alloc_index_mem()
{
    ...
    {
        while ( (shmid = ::shmget( _nIpcKey + 4 * MAX_MEM_BLKS + i, 0, 0400 )) < 0 )
        {
          usleep(10);
          ntry ++;
          if ( ntry > 1000 )
            throw bad_msg(__FILE__,__LINE__,283,(_("ERROR: shmget failed. key = {1}") % (_nIpcKey + 4 * MAX_MEM_BLKS + i)).str(SGLOCALE));
        }
