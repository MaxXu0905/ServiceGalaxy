	...
	for(i = 0 ; i < MAX_MEM_BLKS; i ++ )
	{
		if ( _nDataShmId[i] >= 0 )
		{
			if ( ::shmdt(_pDataAddr[i]) < 0 )
				throw bad_msg(__FILE__,__LINE__,285,(_("ERROR: shmdt failed.")).str(SGLOCALE));
			if ( ::shmctl(_nDataShmId[i],IPC_RMID,0) < 0 )
				throw bad_msg(__FILE__,__LINE__,285,(_("ERROR: shmctl failed.")).str(SGLOCALE));
			_pDataAddr[i] = 0;
		}
		if ( _nHashShmId[i] >= 0 )
		{
...
void sharememserver::dump_super_block()
{
	int i = 0,j = 0;
	//dump obj
	cout  << "\n\n" << (_("dump object memory...")).str(SGLOCALE) << "\n";
	for( i = 0; i < _nSize; i ++ )
	{
		printf("tableindex:[%d]\n",i);
	...
	cout  << "\n\n" << (_("dump hash memory...")).str(SGLOCALE) << "\n";
	//dump hash
	for( i = 0; i < _nSize; i ++ )
	{
...
{
	_nManageShmId = ::shmget( _nIpcKey, 0, 0660 );
	...
	if ( _nManageShmId < 0 )
	{
		_nManageShmId = ::shmget( _nIpcKey, _nSize* _nShmRecordSize, IPC_CREAT | 0660);
		if ( _nManageShmId < 0 )
		{
			throw bad_msg(__FILE__,__LINE__,283,(_("ERROR: create super block share memory failed.")).str(SGLOCALE));
		}
		_pobjshm =  (super_block*) ::shmat( _nManageShmId, 0, 0 );
#if defined(HPUX)
		if ( _pobjshm == SHM_FAILED )
		{
			throw bad_msg(__FILE__,__LINE__,284,(_("ERROR: attach super block share memory failed.")).str(SGLOCALE));
		}
#else
		if ( _pobjshm < 0 )
		{
...
int sharememserver::alloc_object_mem()
{
	...
	for( i = 0 ; i < _nSize; i ++ )
	{
		...
		if ( obj_cnt_lft > 0 )
		{
			...
			{
				throw bad_msg(__FILE__,__LINE__,286,(_("ERROR: MAX_MEM_BLKS too small,[{1}] > [{2}]")% obj_blk_num % MAX_MEM_BLKS ).str(SGLOCALE));
			}
	...
	if (  (blk_index + 1 ) > MAX_MEM_BLKS )
		throw bad_msg(__FILE__,__LINE__,286,(_("ERROR: MAX_MEM_BLKS too small, {1} > {2}") % (blk_index + 1) % MAX_MEM_BLKS).str(SGLOCALE));
	...
	{
		_nDataShmId[i] = ::shmget( _nIpcKey + 2 * MAX_MEM_BLKS + i, 0, 0660 );
		if ( _nDataShmId[i] >= 0 )
		{
			if (::shmctl(_nDataShmId[i],IPC_RMID,0) < 0)
				throw bad_msg(__FILE__,__LINE__,285,(_("ERROR: shmctl failed.")).str(SGLOCALE));
		...
		_nDataShmId[i] = ::shmget( _nIpcKey + 2 * MAX_MEM_BLKS + i, BLOCK_SIZE, IPC_CREAT | 0660);
		if ( _nDataShmId[i] < 0 )
			throw bad_msg(__FILE__,__LINE__,283,(_("ERROR: create data share memory failed.")).str(SGLOCALE));

		_pDataAddr[i] =  (char*)::shmat( _nDataShmId[i], 0, 0 );
#if defined(HPUX)
		if ( _pDataAddr[i] == SHM_FAILED )
			throw bad_msg(__FILE__,__LINE__,284,(_("ERROR: attach data share memory failed.")).str(SGLOCALE));
#else
		if ( _pDataAddr[i] < 0 )
			throw bad_msg(__FILE__,__LINE__,284,(_("ERROR: attach data share memory failed.")).str(SGLOCALE));
#endif
	...
	if ( offset > 0 )
	{
		_nDataShmId[blk_index] = ::shmget( _nIpcKey + 2 * MAX_MEM_BLKS + blk_index, 0, 0660 );
		...
		_nDataShmId[blk_index] = ::shmget( _nIpcKey + 2 * MAX_MEM_BLKS + blk_index, offset, IPC_CREAT | 0660);
		if ( _nDataShmId[blk_index] < 0 )
			throw bad_msg(__FILE__,__LINE__,283,(_("ERROR: create data share memory failed.")).str(SGLOCALE));
...
int sharememserver::alloc_index_mem()
{
	...
	for( i = 0 ; i < _nSize ; i ++ )
	{
		...
		if ( hash_cnt_lft > 0 )
		{
			...
			if ( hash_blk_num > MAX_MEM_BLKS )
				throw bad_msg(__FILE__,__LINE__,286,(_("ERROR: MAX_MEM_BLKS too small, {1} > {2}") % hash_blk_num % MAX_MEM_BLKS).str(SGLOCALE));
	...
	if (is_allocated)
	{
		...
		for ( i = 0 ; i < blk_index ; i ++ )
		{
			_nHashShmId[i] = ::shmget( _nIpcKey + 4  * MAX_MEM_BLKS + i, 0, 0660 );
			...
			_nHashShmId[i] = ::shmget( _nIpcKey + 4  * MAX_MEM_BLKS + i, BLOCK_SIZE, IPC_CREAT | 0660);
			if ( _nHashShmId[i] < 0 )
			{
				throw bad_msg(__FILE__,__LINE__,283,(_("ERROR: create index share memory failed.")).str(SGLOCALE));
			}
			_pHashAddr[i] = (char*)::shmat( _nHashShmId[i], 0, 0 );
#if defined(HPUX)
			if ( _pHashAddr[blk_index] == SHM_FAILED )
			{
				throw bad_msg(__FILE__,__LINE__,284,(_("ERROR: attach index share memory failed.")).str(SGLOCALE));
			}
#else
		...
		if ( offset > 0 )
		{
			_nHashShmId[blk_index] = ::shmget( _nIpcKey + 4  * MAX_MEM_BLKS + blk_index, 0, 0660 );
			...
			_nHashShmId[blk_index] = ::shmget( _nIpcKey + 4  * MAX_MEM_BLKS + blk_index, offset, IPC_CREAT | 0660);
			if ( _nHashShmId[blk_index] < 0 )
			{
				throw bad_msg(__FILE__,__LINE__,283,(_("ERROR: create index share memory failed.")).str(SGLOCALE));
			}
