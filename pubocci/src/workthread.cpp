...
threadrwlock::threadrwlock()
{
	...
	if (pthread_rwlock_init(&_lock, &attr) < 0)
		throw bad_param(__FILE__,__LINE__,316, (_("ERROR: pthread_rwlock_init error.")).str(SGLOCALE));
}
...
threadmutex::threadmutex()
{
	...
	if (pthread_mutex_init(&_mutex, &_attr))
		throw bad_param(__FILE__,__LINE__,317, (_("ERROR: pthread_mutex_init error.")).str(SGLOCALE));
}
...
threadcond::threadcond() : threadmutex()
{
	if (pthread_cond_init(&_cond, 0))
		throw bad_param(__FILE__,__LINE__,318, (_("ERROR: pthread_cond_init error.")).str(SGLOCALE));
}
...
int workthread::start(int nCallBack)
{
	...
	case 2:
		...
		break;
	default:
		throw bad_msg(__FILE__,__LINE__,319, (_("ERROR: create thread failed.")).str(SGLOCALE));
	}
