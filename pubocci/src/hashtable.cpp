...
int hashtable::get_block_info(int index,int& memblkindex,int& blkindex)
{
  ...
  {
    throw bad_msg(__FILE__,__LINE__,216,(_("ERROR: index = {1} exceed hash size = {2}") %index % hashsize).str(SGLOCALE));
  }

  if ( hashblocknum < 1 )
  {
    throw bad_msg(__FILE__,__LINE__,217,(_("ERROR: block number < 1.")).str(SGLOCALE));
  }
  ...
  if ( hashblocknum == 1 )
  {
    if ( index > m_hashblk[0].hash_end || index < m_hashblk[0].hash_start  )
    {
      blkindex = -1;
      memblkindex = -1;
      throw bad_msg(__FILE__,__LINE__,216,(_("ERROR: index exceed block range.")).str(SGLOCALE));
    }
  ...
  while ( nLast >= nBegin )
  {
    ...
    else
    {
      ...
      if ( blkindex < 0 || blkindex >= totalhashblocknum )
        throw bad_msg(__FILE__,__LINE__,218,(_("ERROR: index exceed block range.")).str(SGLOCALE));
      return 0;
    }
  }
  memblkindex = -1;
...
int hashtable::add_index(const char* key,const int index)
{
  ...
  int memblkindex,blkindex;
  if ( key == 0 )
    throw bad_msg(__FILE__,__LINE__,218,(_("ERROR: key is null.")).str(SGLOCALE));
