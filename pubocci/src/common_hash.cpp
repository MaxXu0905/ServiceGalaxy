...
common_hash::common_hash(size_t hash_size, size_t max_data_count, size_t unit_size, compar_func_t in_compar, getkey_func_t in_getkey)
{
    ...
    if (hash_size <= 0) {
        throw bad_param(__FILE__, __LINE__, 100, (_("ERROR: hash_size must bigger than 0.")).str(SGLOCALE));
    }
    if (max_data_count <= 0) {
        throw bad_param(__FILE__, __LINE__, 101, (_("ERROR: max_data_count must bigger than 0.")).str(SGLOCALE));
    }
    if (unit_size <= 0) {
        throw bad_param(__FILE__, __LINE__, 102, (_("ERROR: unit_size must bigger than 0.")).str(SGLOCALE));
    }
...
common_hash::common_hash(size_t hash_size, size_t unit_size,compar_func_t in_compar, getkey_func_t in_getkey,bool save_memory)
{
    ...
    if (unit_size <= 0) {
        throw bad_param(__FILE__, __LINE__, 100, (_("ERROR: unit_size must bigger than 0.")).str(SGLOCALE));
    }
...
bool common_hash::insert(const hash_unit_t* unit)
{
    ...
    if ((occup = alloc_record()) == NULL)
        throw bad_msg(__FILE__, __LINE__, 105, (_("ERROR: max_data_count is too small.")).str(SGLOCALE));
...
{
    if (index == NULL) {
        throw bad_msg(__FILE__, __LINE__, 107, (_("ERROR: illegal index.")).str(SGLOCALE));
    }
